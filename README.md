...
### 📝 会话管理
- **自定义会话名称** - 为会话设置易记的显示名称，方便识别
- **会话内搜索** - 在会话详情中搜索消息内容，支持上下跳转导航
- **搜索关键词同步** - 从全局搜索结果进入会话时，自动填充搜索词并定位
- **项目内搜索** - 在项目详情页快速筛选会话（按名称或 ID）
...
#### 前端优化
- **React Query** - 智能数据获取和缓存，自动重试、乐观更新
- **组件懒加载** - React.lazy + Suspense 实现路由级代码分割
- **虚拟列表** - react-window 实现，支持 >10,000 条记录流畅渲染
- **状态管理拆分** - 细粒度 Zustand store，减少重渲染
- **组件级缓存** - 使用 React.memo 避免不必要重渲染
- **Vite 优化构建** - 代码分割、Terser 压缩、现代浏览器目标
- **前端防抖** - 30秒内不重复请求项目列表
- **并发保护** - 防止同时发起多个扫描请求
...
1. 在项目详情页的会话列表中，点击会话名称旁的小箭头
2. 输入自定义名称（如"修复登录bug"）
3. 按 Enter 保存，或按 ESC 取消
4. 自定义名称将替代原始会话 ID 显示

### 会话内搜索

1. 进入会话详情后，在顶部搜索框输入关键词
2. 使用上下箭头按钮或快捷键导航匹配结果：
   - `Enter` - 跳转到下一个匹配
   - `Shift + Enter` - 跳转到上一个匹配
   - `ESC` - 清除搜索
3. 匹配的消息会自动高亮并滚动到可视区域

### 项目内搜索会话

1. 在项目详情页顶部，使用"搜索会话..."输入框
2. 输入会话名称或 ID 片段进行筛选
3. 列表会实时显示匹配的会话数量
4. 点击清空按钮恢复显示全部会话
...
| `/api/sessions/:project/:session/display-name` | POST | 设置会话显示名称 |
